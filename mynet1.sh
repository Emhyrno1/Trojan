...
EOF
    ...
    docker pull caddy:2.6.2 &&
      docker run --network=mynet1 -d --name trojan-panel-caddy --restart always \
        --network=host \
...
EOF
    ...
    docker pull nginx:1.20-alpine &&
      docker run --network=mynet1 -d --name trojan-panel-nginx --restart always \
        --network=host \
  ...
  if [[ -z $(docker ps -a -q -f "name=^trojan-panel-mariadb$") ]]; then
    ...
    if [[ "${mariadb_user}" == "root" ]]; then
      docker pull mariadb:10.7.3 &&
        docker run --network=mynet1 -d --name trojan-panel-mariadb --restart always \
          --network=host \
...
install_redis() {
  if [[ -z $(docker ps -a -q -f "name=^trojan-panel-redis$") ]]; then
    ...
    docker pull redis:6.2.7 &&
      docker run --network=mynet1 -d --name trojan-panel-redis --restart always \
        --network=host \
        redis:6.2.7 \
...
EOF
    ...
    docker pull jonssonyan/trojan-panel-ui:2.1.5 &&
      docker run --network=mynet1 -d --name trojan-panel-ui --restart always \
        --network=host \
...
install_trojan_panel() {
  if [[ -z $(docker ps -a -q -f "name=^trojan-panel$") ]]; then
    ...
    docker pull jonssonyan/trojan-panel:2.1.4 &&
      docker run --network=mynet1 -d --name trojan-panel --restart always \
        --network=host \
...
install_trojan_panel_core() {
  if [[ -z $(docker ps -a -q -f "name=^trojan-panel-core$") ]]; then
    ...
    docker pull jonssonyan/trojan-panel-core:2.1.1 &&
      docker run --network=mynet1 -d --name trojan-panel-core --restart always \
        --network=host \
